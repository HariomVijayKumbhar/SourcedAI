...
async def upload_document(request: Request, file: UploadFile = File(...), chat_id: str | None = Form(None), user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        filepath = await save_upload(
            file,
            dest_dir=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "uploads"),
    ...
    try:
        ...
        _safe_remove(filepath, settings)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to extract text from file")
...
async def document_count(chat_id: str | None = None, user: dict = Depends(get_current_user)):
    try:
        count = get_document_count(chat_id=chat_id)
        return {"count": count}
    except Exception as e:
        logger.error(f"Error getting document count: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get document count")
